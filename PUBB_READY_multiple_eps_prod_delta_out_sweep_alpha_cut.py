...
epsilons = [1e-4, 1e-5] #0.00001, 0.000001

for jdx, eps in enumerate(epsilons):
    delta_larges = np.logspace(-2-np.log10(eps), 2-np.log10(eps), N)
    ...
    x_vals = eps * delta_larges

    ax.plot(x_vals, huber_err / bo_err, label="eps = {:.7f}".format(eps))
...
ax.set_ylabel(r"$E_{\text{gen}}^{Huber} / E_{\text{gen}}^{BO}$")
ax.set_xscale("log")
ax.set_yscale("log")
