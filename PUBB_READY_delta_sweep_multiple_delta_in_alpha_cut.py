...
np.savetxt(
    "./data/sweep_delta_fixed_eps_{:.2f}_beta_{:.2f}_alpha_cut_{:.2f}_delta_small.csv".format(
        eps, beta, alpha_cut, delta_small
    ), 
    np.vstack((delta_larges, l2_err, l1_err, huber_err, bo_err)).T, 
    delimiter=",",
    header="delta_large,l2,l1,Huber,BO"
)

ax.plot(delta_larges / delta_small, l2_err, label=r"$\ell_2\:\Delta_{\text{\tiny{IN}}}=$"+str(delta_small))
ax.plot(delta_larges / delta_small, l1_err, label=r"$\ell_1\:\Delta_{\text{\tiny{IN}}}=$"+str(delta_small))
ax.plot(delta_larges / delta_small, huber_err, label=r"Huber $\Delta_{\text{\tiny{IN}}}=$"+str(delta_small))
ax.plot(delta_larges / delta_small, bo_err, label=r"BO $\Delta_{\text{\tiny{IN}}}=$"+str(delta_small))
...
ax.set_xlabel(r"$\Delta_\text{\tiny{OUT}}$")
...
# ax.set_xticklabels([r"$10^{-2}$", r"$\Delta_{\text{\tiny{IN}}}$", r"$10^{0}$", r"$10^{1}$", r"$10^{2}$"])
...
if save:
    pu.save_plot(
        fig,
        "sweep_delta_fixed_epsilon_{:.2f}_beta_{:.2f}_alpha_cut_{:.2f}".format(eps, beta, alpha_cut),
    )
