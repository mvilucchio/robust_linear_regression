...
# epsilons = np.logspace(-5, -3, N)
l2_err_ub = np.empty(len(epsilons))
l2_lambda_ub = np.empty(len(epsilons))
l1_err_ub = np.empty(len(epsilons))
l1_lambda_ub = np.empty(len(epsilons))
huber_err_ub = np.empty(len(epsilons))
hub_lambda_ub = np.empty(len(epsilons))
a_hub_ub = np.empty(len(epsilons))
bo_err_ub = np.empty(len(epsilons))
...
for idx, eps in enumerate(tqdm(epsilons)):
    ...
    l2_err_ub[idx], l2_lambda_ub[idx] = _find_optimal_reg_param_gen_error(
        alpha_cut,
        var_func_L2,
        var_hat_func_L2_decorrelated_noise,
        initial_condition,
    ...
    l1_err_ub[idx], l1_lambda_ub[idx] = _find_optimal_reg_param_gen_error(
        alpha_cut,
        var_func_L2,
        var_hat_func_L1_decorrelated_noise,
        initial_condition,
    ...
    huber_err_ub[idx], hub_lambda_ub[idx], a_hub_ub[idx] = _find_optimal_reg_param_and_huber_parameter_gen_error(
        alpha_cut,
        var_hat_func_Huber_decorrelated_noise,
        initial_condition,
        params,
...
#     np.vstack((epsilons, l2_err_ub, l2_lambda_ub, l1_err_ub, l1_lambda_ub, huber_err_ub, hub_lambda_ub, a_hub_ub, bo_err_ub)).T,
#     delimiter=",",
#     header="# alphas_L2, errors_L2, lambdas_L2, errors_L1, lambdas_L1, errors_Huber,lambdas_Huber, huber_params, errors_BO",
# )
...
# ax.plot(epsilons, l2_err_ub, label=r"$\ell_2$ ub.", color="tab:blue")
# ax.plot(epsilons, l1_err_ub, label=r"$\ell_1$ ub.", color="tab:green")
# ax.plot(epsilons, huber_err_ub, label="Huber ub.", color="tab:orange")
...
if save:
    pu.save_plot(
        fig,
        "absolute_sweep_eps_scaling_fixed_delta_{:.2f}_beta_{:.2f}_alpha_cut_{:.2f}_delta_small_{:.2f}".format(
