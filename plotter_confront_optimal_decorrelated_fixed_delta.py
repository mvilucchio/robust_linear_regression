...
for idx, (L2_d, L2_e, L1_d, L1_e, Huber_d, Huber_e, BO_d) in enumerate(
    ...
    error_names.append("Percen. = ${:.2f}$".format(percentages[idx]))
    error_names_latex.append("$\epsilon = {:.2f}$".format(percentages[idx]))
    ...
    idx_mask = lambdas_Huber >= 0.005

    ax20.plot(
        alphas_Huber[idx_mask],
        lambdas_Huber[idx_mask],
        label="Huber",
        linestyle="dashed",
        # color=cmap(idx),
        # marker=marker_cycler[idx],
