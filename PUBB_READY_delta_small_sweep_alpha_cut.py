...
N = 10
# delta_smalls = np.linspace(0.01, 10, N)
delta_smalls = np.logspace(-2, np.log10(delta_large), N)

l2_err = np.empty(len(delta_smalls))
l1_err = np.empty(len(delta_smalls))
huber_err = np.empty(len(delta_smalls))
bo_err = np.empty(len(delta_smalls))

for idx, delta_small in enumerate(delta_smalls):
    while True:
...
ax.plot(delta_smalls, l2_err, label=r"$\ell_2$")
ax.plot(delta_smalls, l1_err, label=r"$\ell_1$")
ax.plot(delta_smalls, huber_err, label="Huber")
ax.plot(delta_smalls, bo_err, label="BO")
ax.set_xlabel(r"$\Delta_\text{\tiny{IN}}$")
ax.set_ylabel(r"$E_{\text{gen}}$")
ax.set_xscale("log")
ax.set_yscale("log")
ax.set_xlim([0.01, 8.0])
...
xt = ax.get_xticks() 
print(xt)
xt = np.append(xt[1:], 5.0)

xtl = xt.tolist()
xtl[-1] = r"$\Delta_{\text{\tiny{OUT}}}$"
ax.set_xticks(xt)
...
if save:
    pu.save_plot(
        fig,
        "sweep_delta_small_fixed_epsilon_{:.2f}_beta_{:.2f}_alpha_cut_{:.2f}".format(eps, beta, alpha_cut),
    )
