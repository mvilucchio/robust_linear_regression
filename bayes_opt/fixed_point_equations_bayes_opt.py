...
def state_equations_bayes_opt(var_hat_func, delta_small=0.1, delta_large=1.0, alpha=0.5, eps=0.1, init=(0.5, 0.5, 0.5)):
    m, q  = init[0], init[1], init[2]
    err = 1.0
    blend = 0.5
    while err > 1e-4:
        q_hat = var_hat_func(q, alpha, delta_small, delta_large, eps=eps)
...
def bayes_opt_theory(var_func, var_hat_func, alpha_1=0.01, alpha_2=100, n_alpha_points=16, lambd=0.1, delta_small=1.0, delta_large=10.0, initial_cond=[0.6, 0.0, 0.0], verbose=False, eps=0.1):
    ...
    for i, alpha in enumerate(tqdm(alphas, desc="alpha", disable=not verbose, leave=False)):
        m, q, _ = state_equations_bayes_opt(
            var_func, var_hat_func, delta_small=delta_small, delta_large=delta_large, alpha=alpha, eps=eps, init=initial
        )
        error_theory[i] = 1 + q - 2 * m
...
def var_hat_func_bayes_opt_eps(q, alpha, delta_small, delta_large, eps=0.1):
    q_hat = alpha * numfuneps.q_hat_equation_bayes_eps(m, q, delta_small, delta_large, eps=eps)
    return q_hat

def var_func_bayes_opt_eps(m_hat, q_hat, sigma_hat, alpha, delta_small, delta_large, lambd):
    m = m_hat / (sigma_hat + lambd)
    q = (np.square(m_hat) + q_hat) / np.square(sigma_hat + lambd)
    sigma = 1.0 / (sigma_hat + lambd)
    return m, q, sigma
...
if __name__ == "__main__":
    ...
    for idx, l in enumerate(tqdm(lambdas, desc="lambda", leave=False)):
        for jdx, (delta_small, delta_large) in enumerate(tqdm(deltas, desc="delta", leave=False)):
            ...
            alphas[i], errors[i] = bayes_opt_theory(
                var_hat_func_bayes_opt_eps,
                alpha_1=alpha_min,
                alpha_2=alpha_max,
    ...
    ax.set_title("Bayes Optimal - Double Noise")
